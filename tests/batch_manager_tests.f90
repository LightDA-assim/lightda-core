  ...
  subroutine test_batch_math()
    ...
    do ibatch=1,n_batches
       ...
       if(ibatch>1.and.batch_offset/=last_offset+batch_length) then
          print *,'Offset incorrect for batch',ibatch,'. Expected',last_offset+batch_length,'got',batch_offset
          error stop
       end if
    end do

    if(sum_batch_lengths/=state_size) then
       print *,'Sum of batch lengths was',sum_batch_lengths,' expected',state_size
       error stop
    end if
