...
80    del = .8333333E-1/fk
      ...
      ELSE
        px = fk*v*v*(((((((a7*v + a6)*v + a5)*v + a4)*v + a3)*v + a2)*v &
                      + a1)*v + a0) - del
      END IF
      py = .3989423/SQRT(fk)
110   x = (half - difmuk)/s
...
!     Local variable
    ...
    fn_val = EXP(lngamma(DBLE(n + 1)) - lngamma(DBLE(r + 1)) &
                 - lngamma(DBLE(n - r + 1)) &
...
!       is the one usually quoted.
    ...
    fn_val = lnrt2pi + arg*(LOG(arg) - 1.d0 + &
                            (((a4*temp + a3)*temp + a2)*temp + a1)*temp) - &
             LOG(product)
    IF (reflect) THEN
      temp = SIN(pi*x)
...
!     ..
!     .. Local Scalars ..
    REAL            :: alv, amaxp, f, f1, f2, u, v, w, w2, x, x1, x2, &
                       ynorm, z, z2
...
!     THE FINAL ACCEPTANCE/REJECTION TEST
      ...
      f2 = f1*f1
      w2 = w*w
      IF (alv - (xm*LOG(f1/x1) + (n - m + half)*LOG(z/w) + &
                 (ix - m)*LOG(w*p/(x1*q)) + &
