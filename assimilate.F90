...
contains
  ...
  subroutine localize(istep, ibatch, dim_p, dim_obs, HP_p, HPH, mgr)
    ...
    class default
      print *, 'Could not determine argument type. Should be class &
           &assim_batch_manager'
    end select
  ...
  subroutine assimilate_parallel( &
    batch_manager, assimilator, istep, n_observations, n_obs_batch_max)
    ...
    real(kind=8), allocatable::obs_errors(:)
    ...
    do ibatch_local = 1, n_local_batches
      ...
      call model_interface%get_subset_predictions(istep, batch_offset, &
                                                  batch_size, predictions)
      call model_interface%get_subset_observations(istep, batch_offset, &
                                                   batch_size, observations)
      call model_interface%get_subset_obs_err(istep, batch_offset, batch_size, &
                                              obs_errors)
      call model_interface%get_innovations( &
        istep, batch_offset, batch_length, &
        observations, predictions, obs_errors, innovations)
